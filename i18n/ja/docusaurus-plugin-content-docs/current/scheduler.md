...
Both implement `IArmActionArgs`. Fields passed to `OnRequestTrasferAction` and `OnTrasferActionCompleted`:
...
| `SubstarteKey` | `string` | Key of the substrate being transferred |
...
```csharp
Scheduler.OnRequestTrasferAction += (DualArmActionArgs args) =>
{
...
```csharp
Scheduler.OnTrasferActionCompleted += (DualArmActionArgs args) =>
{
