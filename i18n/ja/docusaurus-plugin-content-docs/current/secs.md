...
**シナリオエディタ**（SECS → SCENARIO）は、すべてのメッセージ本文を
**Body (PORT SML)** フィールドにコンパクトな JSON 値で表現します。この JSON は
シナリオテスト実行時に port サーバーが Go HSMS ライブラリと gRPC でやり取りする値と
同一であり、エディタに入力した内容がそのまま送信バイトになります。

空の本文（S1F1 のようなヘッダのみのメッセージ）は空欄にするか `[]` と記述します。

**自動型マッピング** — プレーンな JSON 値は値に応じて SECS-II アイテムに
エンコードされます:

| JSON 値 | SECS-II 型 | 備考 |
|---------|-----------|------|
| `[ ... ]`（配列） | `L`（List） | ネストした配列はネストしたリストになります |
| `"text"`（文字列） | `A`（ASCII） | |
| `true` / `false` | `BOOLEAN` | |
| 整数 `0 … 255` | `U1` | 収まる最小の符号なし型 |
| 整数 `256 … 65535` | `U2` | |
| 整数 `65536 … 4294967295` | `U4` | |
| 整数 `≥ 4294967296` | `U8` | |
| 負の整数 | `I1`/`I2`/`I4`/`I8` | 収まる最小の符号付き型 |
| 整数以外の数 | `F8` | 8バイト浮動小数点 |

**明示的な型指定** — 自動マッピングが望む結果でない場合（例: バイナリ ACK コード、
`U1` に収まる値を `U4` に強制、`F4` の使用など）は、構造化された
`{ "Type": ..., "Value": ... }` 形式を使用します。`Type` は SECS-II 型
トークン（`A`, `B`, `BOOL`, `U1`–`U8`, `I1`–`I8`, `F4`, `F8`, `L`）です:

```json
{ "Type": "B", "Value": 0 }            // ACKC — バイナリ 0
{ "Type": "U4", "Value": 1000 }        // U2 の代わりに U4 を強制
{ "Type": "L", "Value": [ { "Type": "A", "Value": "START" } ] }
```

**例** — 左が PSML 構造、右が同等の Body (PORT SML) です:

| メッセージ | PSML 構造 | Body (PORT SML) |
|-----------|----------|---------------------|
| S1F1 (Are You There) | `{}` | *(空)* |
...
> 上記の Body (PORT SML) 値は、シナリオエディタが
> `POST /api/v1/secs/scenario/send` で Passive モードの装置に送信する値であり、
> 装置の応答も同じ JSON 形式で返されます。フィールドごとの構造は以下の各メッセージの
> **Parameters** テーブルを参照してください。
