...
> 注: `m` パラメータは任意です。`[Handler]` が型付きハンドラー（`IModelFlowHandler<T>` など）の場合、ステップは `m` を省略し、ハンドラー経由で `Handler.Model`（3 型ハンドラーでは `Handler.GetModel()`）からモデルを読み取れます。どちらも同じモデルシングルトンに解決されます。
