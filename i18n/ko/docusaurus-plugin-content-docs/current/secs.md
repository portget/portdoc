...
**시나리오 편집기**(SECS → SCENARIO)는 모든 메시지 본문을 **Body (PORT SML)**
필드에 간결한 JSON 값으로 표현합니다. 이 JSON은 시나리오 테스트 실행 시 port
서버가 Go HSMS 라이브러리와 gRPC로 주고받는 값과 동일하므로, 편집기에 입력한
그대로가 실제 전송 바이트가 됩니다.

빈 본문(S1F1 처럼 헤더만 있는 메시지)은 비워 두거나 `[]` 로 씁니다.

**자동 타입 매핑** — 순수 JSON 값은 값에 따라 SECS-II 아이템으로 인코딩됩니다:

| JSON 값 | SECS-II 타입 | 비고 |
|---------|--------------|------|
| `[ ... ]` (배열) | `L` (List) | 중첩 배열은 중첩 리스트가 됩니다 |
| `"text"` (문자열) | `A` (ASCII) | |
| `true` / `false` | `BOOLEAN` | |
| 정수 `0 … 255` | `U1` | 값이 들어가는 가장 작은 부호 없는 타입 |
| 정수 `256 … 65535` | `U2` | |
| 정수 `65536 … 4294967295` | `U4` | |
| 정수 `≥ 4294967296` | `U8` | |
| 음의 정수 | `I1`/`I2`/`I4`/`I8` | 값이 들어가는 가장 작은 부호 있는 타입 |
| 정수가 아닌 수 | `F8` | 8바이트 실수 |

**명시적 타입 지정** — 자동 매핑이 원하는 결과가 아닐 때(예: 바이너리 ACK 코드,
`U1`에 들어갈 값을 `U4`로 강제, `F4` 사용 등)에는 구조화된
`{ "Type": ..., "Value": ... }` 형식을 사용합니다. `Type`은 SECS-II 타입
토큰(`A`, `B`, `BOOL`, `U1`–`U8`, `I1`–`I8`, `F4`, `F8`, `L`)입니다:

```json
{ "Type": "B", "Value": 0 }            // ACKC — 바이너리 0
{ "Type": "U4", "Value": 1000 }        // U2 대신 U4 강제
{ "Type": "L", "Value": [ { "Type": "A", "Value": "START" } ] }
```

**예시** — 왼쪽은 PSML 구조, 오른쪽은 동등한 Body (PORT SML)입니다:

| 메시지 | PSML 구조 | Body (PORT SML) |
|--------|----------|---------------------|
| S1F1 (Are You There) | `{}` | *(비움)* |
...
> 위의 Body (PORT SML) 값은 시나리오 편집기가
> `POST /api/v1/secs/scenario/send` 로 Passive 모드 장비에 전송하는 값이며,
> 장비의 응답도 동일한 JSON 형식으로 반환됩니다. 필드별 구조는 아래 각 메시지의
> **Parameters** 표를 참고하세요.
...
| | | 0: Acknowledged |
| | | 1: Error |
| Comment | - | The standards have had an erroneous structure for years - the L[2] has been missing. Unfortunately some implementations have not realized it was an error. The latest  versions automagically create the L[2] wrapper when it is missing. |
