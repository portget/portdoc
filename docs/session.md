...
PortDIC provides an operator session management handler (`ISessionHandler`) for SEMI E30 compliant systems. It handles user authentication (login/logout), role-based access control, and Control State transitions (Offline / Local / Remote). Passwords are hashed with bcrypt.
...
- Passwords are **never stored in plaintext** — bcrypt hashing is applied automatically.
- The singleton pattern ensures all `[Auth]` classes share one session state — a login from one component is visible to all.
- Role-level enforcement for Control State transitions is applied automatically.
